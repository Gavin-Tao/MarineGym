#!/bin/bash
# TTE实验: T2-T5 (A+B-soft, 不同soft_hi) — 使用默认MPPI参数
# 不传 num_samples/horizon/risk.horizon, 走代码默认值 (num_samples=128, horizon=20, risk.horizon=15)
set +e
...
# 默认MPPI参数 (不传 num_samples/horizon/risk.horizon, 走默认)
ABSOFT="task.keepout.enable=true task.keepout.radius=0.8 task.keepout.dynamic.enable=true task.keepout.mppi.enable=true task.keepout.mppi.exact=true task.keepout.risk.enable=true task.keepout.mppi.soft_blend=true"
